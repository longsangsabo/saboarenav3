...
class CompleteSaboDE16Service {
  ...
  Future<Map<String, dynamic>> generateSaboDE16Bracket({
    ...
    try {
      debugPrint('$_tag: Generating SABO DE16 bracket for tournament $tournamentId');
      ...
      await _generateCompleteBracketWithMatchProgression(tournamentId, participants);
  ...
  Future<Map<String, dynamic>> processMatchCompletion(
    ...
    try {
      ...
      debugPrint('$_tag: Match ${completedMatch['id']} - Round ${completedMatch['round_number']}');
      ...
      if (roundNumber == 4) {
        ...
        debugPrint('$_tag: Winners Bracket stops at R3, goes directly to SEMI1/SEMI2');
      ...
      switch (roundNumber) {
        ...
        case 1:
          await _processWR1Completion(tournamentId, completedMatch, winnerId, results);
          break;
        case 2:
          await _processWR2Completion(tournamentId, completedMatch, winnerId, results);
          break;
        case 3:
          await _processWR3Completion(tournamentId, completedMatch, winnerId, results);
          break;
          
        // Losers Branch A Rounds
        case 101:
          await _processLAR101Completion(tournamentId, completedMatch, winnerId, results);
          break;
        case 102:
          await _processLAR102Completion(tournamentId, completedMatch, winnerId, results);
          break;
        case 103:
          await _processLAR103Completion(tournamentId, completedMatch, winnerId, results);
          break;
          
        // Losers Branch B Rounds
        case 201:
          await _processLBR201Completion(tournamentId, completedMatch, winnerId, results);
          break;
        case 202:
          await _processLBR202Completion(tournamentId, completedMatch, winnerId, results);
          break;
          
        // SABO Finals Rounds
        case 250:
        case 251:
          await _processSaboSemifinalCompletion(tournamentId, completedMatch, winnerId, results);
          break;
        case 300:
          await _processSaboFinalCompletion(tournamentId, completedMatch, winnerId, results);
          break;
          
        default:
          debugPrint('$_tag: ❌ Unknown round number $roundNumber');
          debugPrint('$_tag: Valid SABO DE16 rounds: 1,2,3,101,102,103,201,202,250,251,300');
      }
  ...
  Future<void> _processWR1Completion(
    ...
    debugPrint('$_tag: Processing WR1 completion - Match ${completedMatch['match_number']}');
  ...
  Future<void> _processWR2Completion(
    ...
    debugPrint('$_tag: Processing WR2 completion - Match ${completedMatch['match_number']}');
  ...
  Future<void> _processWR3Completion(
    ...
    debugPrint('$_tag: Processing WR3 completion - Match ${completedMatch['match_number']}');
    ...
    final saboUpdated = await _advancePlayerToMatch(
      tournamentId, winnerId, saboSemifinalRound, saboMatchNumber, 'WR3M$matchNumber'
    );
  ...
  Future<void> _processLAR101Completion(
    ...
    debugPrint('$_tag: Processing LAR101 completion - Match ${completedMatch['match_number']}');
  ...
  Future<void> _processLAR102Completion(
    ...
    debugPrint('$_tag: Processing LAR102 completion - Match ${completedMatch['match_number']}');
    
    final nextMatches = <Map<String, dynamic>>[];
    
    // Both LAR102 winners advance to LAR103 M21
    final lar103Updated = await _advancePlayerToMatch(
      tournamentId, winnerId, 103, 21, 'LAR102M${completedMatch['match_number']}'
    );
  ...
  Future<void> _processLAR103Completion(
    ...
    debugPrint('$_tag: Processing LAR103 completion - Branch A Champion: $winnerId');
  ...
  Future<void> _processLBR201Completion(
    ...
    debugPrint('$_tag: Processing LBR201 completion - Match ${completedMatch['match_number']}');
    
    final nextMatches = <Map<String, dynamic>>[];
    
    // Both LBR201 winners advance to LBR202 M24
    final lbr202Updated = await _advancePlayerToMatch(
      tournamentId, winnerId, 202, 24, 'LBR201M${completedMatch['match_number']}'
    );
  ...
  Future<void> _processLBR202Completion(
    ...
    debugPrint('$_tag: Processing LBR202 completion - Branch B Champion: $winnerId');
  ...
  Future<void> _processSaboSemifinalCompletion(
    ...
    debugPrint('$_tag: Processing SABO Semifinal completion - Round ${completedMatch['round_number']}');
    
    final nextMatches = <Map<String, dynamic>>[];
    
    // Both semifinal winners advance to SABO Final (R300 M27)
    final roundNumber = completedMatch['round_number'] as int;
    final finalUpdated = await _advancePlayerToMatch(
      tournamentId, winnerId, 300, 27, 'SEMI${roundNumber == 250 ? "1" : "2"}'
    );
  ...
  Future<Map<String, dynamic>?> _advancePlayerToMatch(
    ...
    try {
      ...
      if (match['player1_id'] == playerId || match['player2_id'] == playerId) {
        debugPrint('$_tag: ❌ ERROR - Player $playerId already assigned to R${roundNumber}M${matchNumber}');
      ...
      debugPrint('$_tag: Advanced player $playerId to R${roundNumber}M${matchNumber} ($updateField) from $sourceMatch');
      ...
      debugPrint('$_tag: Error advancing player to R${roundNumber}M${matchNumber}: $e');
      return null;
    }
  }
  ...
  List<Map<String, dynamic>> _generateAllSaboDE16Matches(String tournamentId) {
    ...
    final lbA_rounds = [101, 102, 103];
    final lbA_counts = [4, 2, 1];
    int matchCounter = 15;
    
    for (int i = 0; i < lbA_rounds.length; i++) {
      for (int j = 1; j <= lbA_counts[i]; j++) {
        matches.add({
          'id': uuid.v4(),
          'tournament_id': tournamentId,
          'round_number': lbA_rounds[i],
    ...
    final lbB_rounds = [201, 202];
    final lbB_counts = [2, 1];
    
    for (int i = 0; i < lbB_rounds.length; i++) {
      for (int j = 1; j <= lbB_counts[i]; j++) {
        matches.add({
          'id': uuid.v4(),
          'tournament_id': tournamentId,
          'round_number': lbB_rounds[i],
          'match_number': matchCounter,
          'player1_id': null,
          'player2_id': null,
          'status': 'pending',
  ...
  Future<void> _saveMatchesToDatabase(List<Map<String, dynamic>> matches) async {
    try {
  ...
  Future<void> _populateR1WithParticipants(
    ...
    try {
      debugPrint('$_tag: Populating R1 with ${participants.length} participants');
      ...
      if (r1Matches.length != 8) {
        throw Exception('SABO DE16 R1 should have exactly 8 matches, found ${r1Matches.length}');
      }
      
      // Sort participants by seed number
      participants.sort((a, b) => (a['seed_number'] as int).compareTo(b['seed_number'] as int));
      ...
      for (int i = 0; i < r1Matches.length; i++) {
        ...
        if (player1Index < participants.length && player2Index < participants.length) {
          ...
          debugPrint('$_tag: R1M${match['match_number']}: ${player1['full_name']} vs ${player2['full_name']}');
        }
      }
  ...
  Future<void> _generateCompleteBracketWithMatchProgression(
    ...
    try {
      debugPrint('$_tag: 🔥 HARDCORE MODE: Generating complete bracket with match progression');
      ...
      debugPrint('$_tag: 🔥 HARDCORE MODE: Complete bracket structure created!');
      debugPrint('$_tag: R1 populated with participants, other rounds await natural progression');
  ...
  Future<void> _createAllMatchesWithProgression(
    ...
    debugPrint('$_tag: Created all ${matches.length} matches with proper structure');
  }
  ...
  Future<void> _simulateCompleteMatchProgression(
    ...
    try {
      debugPrint('$_tag: 🔥 HARDCORE: Simulating complete bracket progression...');
      ...
      debugPrint('$_tag: 🔥 HARDCORE: All rounds populated with simulated progression');
  ...
  Future<void> _populateWithCorrectProgression(
    ...
    try {
      debugPrint('$_tag: 🔥 HARDCORE: Populating with correct bracket progression...');
      ...
      debugPrint('$_tag: 🔥 HARDCORE: All matches populated with CORRECT progression logic');
  ...
  Future<void> _simulateR1AndPopulateCorrectly(
    ...
    debugPrint('$_tag: Populated R2 with ${r2Matches.length} matches based on R1 progression');
    debugPrint('$_tag: Populated LB R101 with ${lbR101Players.length} losers from R1');
  }

  /// Get simulated winner based on seeding (higher seed wins)
  String _getSimulatedWinner(Map<String, dynamic> match, List<Map<String, dynamic>> participants) {
    final player1Id = match['player1_id'];
    final player2Id = match['player2_id'];
    
    if (player1Id == null || player2Id == null) return player1Id ?? player2Id ?? '';
  ...
  /// Get simulated loser
  String _getSimulatedLoser(Map<String, dynamic> match, List<Map<String, dynamic>> participants) {
    final player1Id = match['player1_id'];
    final player2Id = match['player2_id'];
  ...
  Future<void> _populateRound1WithParticipants(
    ...
    try {
      ...
      debugPrint('$_tag: Populated ${r1Matches.length} R1 matches with proper seeding');
    ...
    try {
      ...
      for (int i = 0; i < matches.length && i * 2 < playerIds.length; i++) {
        ...
        await _supabase
            .from('matches')
            .update({
              'player1_id': player1Index < playerIds.length ? playerIds[player1Index] : null,
              'player2_id': player2Index < playerIds.length ? playerIds[player2Index] : null,
            })
            .eq('id', match['id']);
      }
      
      debugPrint('$_tag: Populated Round $roundNumber with ${playerIds.length} players');
  ...
  Future<void> _populateSaboFinalsWithChampions(
    ...
    try {
      ...
      await _populateMatchByRound(tournamentId, 250, results['wr3_winners'][0], results['lar103_winner']);
      
      // SEMI2 (R251): WR3 winner vs LBR202 winner  
      await _populateMatchByRound(tournamentId, 251, results['wr3_winners'][1], results['lbr202_winner']);
      
      // FINAL (R300): SEMI1 winner vs SEMI2 winner
      await _populateMatchByRound(tournamentId, 300, results['semi1_winner'], results['semi2_winner']);
