...
# Use the sabo1 tournament that has the issue
tournament_id = 'cf969300-a9db-44f1-8b4e-c27d5ece2c75'  # Full sabo1 tournament ID
tournaments = supabase.table('tournaments').select('id').eq('id', tournament_id).execute()
if tournaments.data:
...
if len(losers_from_wb_r1) > 0 and len(empty_lb_slots) > 0:
    ...
    lb_r1_after = supabase.table('matches').select('*').eq('tournament_id', tournament_id).eq('round_number', 101).execute()
